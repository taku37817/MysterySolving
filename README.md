...
| ファイル名                                       | 説明 |
|--------------------------------------------------|------|
| **HoloLensEngine.ini** | HoloLens（Microsoft MRデバイス）向けのエンジン設定ファイルです。<br>プロジェクトがHoloLensを対象にしていない場合は特に使われないことが多いです。自動生成された可能性あります。 |
| **DefaultEditor.ini** | Unreal Editorの設定（UI、ツールバー表示、プラグイン設定など）を定義してます。開発中のエディタ挙動に関係します。 |
